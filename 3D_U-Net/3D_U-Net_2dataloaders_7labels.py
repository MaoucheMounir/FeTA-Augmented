    ...
    for i in range(0, image.shape[0] - patch_size[0] + 1, stride):
        for j in range(0, image.shape[1] - patch_size[1] + 1, stride):
            for k in range(0, image.shape[2] - patch_size[2] + 1, stride):
                ...
                if np.count_nonzero(seg_patch == 4) >= pourcentage * np.prod(patch_size):
...
#     nib.save(nib.Nifti1Image(seg_patch, np.eye(4)), f"/Users/garance/Desktop/test/seg_patch_{i}.nii.gz")
...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(config['num_epochs'])):
        ...
        for batch in zip(train_loader1, train_loader2):
            ...
            if step <= 500/int(config['nb_dataloaders']):   # in 1 epoch, the model will see 500/nb_dataloaders patches randomly selected
                print('Step :', step)
                model.train()
                optimizer.zero_grad()
